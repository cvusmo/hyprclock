...
pub fn validate_animations(animations: &[AnimationConfig]) -> Result<(), String> {
    ...
    for animation in animations {
        ...
        if animation.blur_enabled && !active_animations.insert("blur") {
            return Err("Error: 'blur' is set to true multiple times.".to_string());
        }

        if animation.fade_in_enabled && !active_animations.insert("fade_in") {
    ...
    let has_blur_enabled = animations.iter().any(|a| a.blur_enabled);
    let has_blur_disabled = animations.iter().any(|a| !a.blur_enabled);
    if has_blur_enabled && has_blur_disabled {
        return Err("Error: 'blur' animation cannot be both true and false.".to_string());
    }

    let has_fade_in_enabled = animations.iter().any(|a| a.fade_in_enabled);
    let has_fade_in_disabled = animations.iter().any(|a| !a.fade_in_enabled);
